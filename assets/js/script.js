...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const titleEl = document.getElementById('titulo-confirmacao');
    const messageEl = document.getElementById('mensagem-confirmacao');
    const keywordEl = document.getElementById('palavra-chave-confirmacao');
    ...
    const handleInput = () => {
        confirmBtn.disabled = inputEl.value !== keyword;
    };
    const closeModal = () => {
    ...
    modal.querySelector('.cancel-btn').addEventListener('click', closeModal, { once: true });
    modal.querySelector('.modal-close-btn').addEventListener('click', closeModal, { once: true });
}

function openTerminateModal(func) {
    const modal = document.getElementById('modal-encerrar-contrato');
    const form = document.getElementById('form-encerrar-contrato');
    document.getElementById('nome-funcionario-encerrar').textContent = func.nome;
    
    const confirmInput = document.getElementById('input-encerrar-confirmacao');
    const submitBtn = form.querySelector('.confirm-btn');

    document.getElementById('data_demissao').value = new Date().toISOString().split('T')[0];
    ...
    const handleInput = () => {
        submitBtn.disabled = confirmInput.value !== 'CONFIRMAR';
    };
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('id', func.id);
        formData.append('data_demissao', document.getElementById('data_demissao').value);
        formData.append('motivo_demissao', document.getElementById('motivo_demissao').value);
        formData.append('elegivel_recontratacao', document.getElementById('elegivel_recontratacao').value);
            ...
            switch (actionTarget.dataset.action) {
                ...
                case 'delete':
                    showConfirmationModal({
                        ...
                        message: `Você está prestes a excluir permanentemente o registro de "${func.nome}". Esta ação não pode ser desfeita.`,
                        keyword: 'EXCLUIR',
                        onConfirm: async () => {
                            const formData = new FormData();
    ...
    if(loginForm) {
        loginForm.addEventListener('submit', async (e) => {
          ...
          try {                              //Alterem a url no ambiente de desenvolvimento de vcs, no caso a porta do localhost para 80 ex: localhost:80
            const response = await fetch('http://localhost:8888/tranquility/backend/api.php?action=login', { 
                method: 'POST', 
                body: formData 
            });
            ...
            showToast(result.message || 'Login efetuado com sucesso!', 'success');
