// assets/js/script.js - Versão Final Completa com Integração Control iD
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDashboardListeners() {
        ...
        // --- LISTENER DO BOTÃO SINCRONIZAR CONTROL ID ---
        const syncBtn = document.getElementById('sync-rhid-btn');
        if (syncBtn) {
            syncBtn.addEventListener('click', async () => {
                const date = document.getElementById('date-picker').value;
                const unit = document.getElementById('unit-filter').value;
                const company = document.getElementById('company-filter-dashboard').value;
                ...
                showToast('Conectando à API Control iD...', 'info');

                try {
                    const formData = new FormData();
                    formData.append('action', 'sync_rhid');
                    formData.append('data', date);
                    formData.append('unidade', unit);
                    ...
                    if (result && result.success) {
                        ...
                        document.getElementById('input-presentes').value = stats.presentes;
                        document.getElementById('input-falta_injustificada').value = stats.faltas;
                        document.getElementById('input-afastados').value = stats.afastados;
                        document.getElementById('input-folga').value = stats.folga;
                        document.getElementById('input-ferias').value = stats.ferias;
                        document.getElementById('input-atestado').value = stats.atestado;

                        // Atualiza o gráfico
                        updateChartAndTable();
                        
                        showToast(result.message || 'Dados sincronizados com sucesso!');
                    } else {
                        // Se falhar a sincronização, não faz nada com os inputs (mantém o que estava)
                        showToast('Não foi possível obter dados detalhados. Verifique a conexão.', 'error');
                    }
                } catch (error) {
                    console.error(error);
